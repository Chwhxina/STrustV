...
public class EventLogPanel extends JPanel
  ...
  /** Number of events -setting id ({@value}). Defines the number of events to show in the panel. */
  public static final String NROF_EVENTS_S = "nrofEvents";

  /**
   * Regular expression filter -setting id ({@value}). Defines the regular expression against which
   * the event texts are matched; only matching events are not shown
   */
  ...
  private final int maxNrofEvents;
   ...
   *
   * @param check EventLogControls used to check if this entry type should be shown and/or paused
   *     upon
