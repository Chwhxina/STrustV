...
public class GUIControls extends JPanel implements ActionListener, ChangeListener {
  /**
   * GUI update speeds. Negative values -> how many 1/10 seconds to wait between updates. Positive
   * values -> show every Nth update
   */
  ...
  private static final String TEXT_SIMTIME =
      "Simulation time - " + "click to force update, right click to change format";
  ...
  private void initPanel() {
    ...
    this.simTimeField.addMouseListener(
        new MouseAdapter() {
    ...
    playButton = addButton(paused ? ICON_PLAY : ICON_PAUSE, paused ? TEXT_PLAY : TEXT_PAUSE);
  ...
  public void setSimTime(double time) {
    ...
    if (this.useHourDisplay) {
      ...
      double secs = time % 60;
      this.simTimeField.setText(String.format("%02d:%02d:%02.1f", hours, mins, secs));
    } else {
   ...
   *
   * @param delta How much to change the current level (can be negative or positive)
   */
  public void changeZoom(int delta) {
    ...
    Number newValue =
        new Double(curZoom + model.getStepSize().doubleValue() * delta * curZoom * 100);
   ...
   *
   * @param centerView If true, the center of the viewport should remain the same
   */
  ...
  private void takeScreenShot() {
    try {
      ...
      if (retVal == JFileChooser.APPROVE_OPTION) {
        ...
        BufferedImage i =
            new BufferedImage(this.pf.getWidth(), this.pf.getHeight(), BufferedImage.TYPE_INT_RGB);
