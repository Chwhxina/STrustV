...
public class PlayField extends JPanel {
   ...
   */
  public void setUnderlayImage(
      BufferedImage image, double dx, double dy, double scale, double rotation) {
    if (image == null) {
      this.underlayImage = null;
      this.imageTransform = null;
    ...
    curTransform.scale(PlayFieldGraphic.getScale(), PlayFieldGraphic.getScale());
  ...
  /**
   * Enables or disables the automatic clearing of overlay graphics. If enabled, overlay graphics
   * are cleared every time a new graphics object is set to be drawn.
   *
  ...
  /**
   * Enables or disables the automatic focus on click. If enabled, the node that is closest to the
   * clicked location is set on focus
   *
  ...
  /**
   * Draws the play field. To be called by Swing framework or directly if different context than
   * screen is desired
   *
  ...
  /**
   * Returns a world location for a given graphical location. Note that there might be inaccuracies
   * because of rounding.
   *
   * @param loc The location to convert
  ...
  private void updateFieldSize() {
    Dimension minSize =
        new Dimension(PlayFieldGraphic.scale(w.getSizeX()), PlayFieldGraphic.scale(w.getSizeY()));
  ...
  /**
   * Sets the focus on the node that is closest to the given coordinates (in the graphic view of the
   * playfield)
   *
