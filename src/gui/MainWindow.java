...
/** Main window for the program. Takes care of layouting the main components in the window. */
...
public class MainWindow extends JFrame {
  ...
  /**
   * Window width -setting id ({@value}). Defines the width of the GUI window. Default {@link
   * #WIN_DEFAULT_WIDTH}
   */
  public static final String WIN_WIDTH_S = "width";
  /**
   * Window height -setting id ({@value}). Defines the height of the GUI window. Default {@link
   * #WIN_DEFAULT_HEIGHT}
   */
  ...
  public MainWindow(
    ...
    hostListScroll.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);

    logControlSplit =
        new JSplitPane(
            JSplitPane.HORIZONTAL_SPLIT, new JScrollPane(elp.getControls()), new JScrollPane(elp));
    logControlSplit.setResizeWeight(0.1);
    logControlSplit.setOneTouchExpandable(true);

    fieldLogSplit = new JSplitPane(JSplitPane.VERTICAL_SPLIT, leftPane, logControlSplit);
    ...
    mainSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, fieldLogSplit, hostListScroll);
