...
/**
 * A Network Interface that that takes in to account he distance from the other (connected)
 * interface when determining the capacity of the links. The distance-dependent transmission speeds
 * are given as comma-separated values using setting {@link #TRANSMIT_SPEEDS_S}.
 */
public class DistanceCapacityInterface extends NetworkInterface {

  /**
   * Comma-separated list of speed values -setting id ({@value} ). The first value is the speed at
   * distance 0 and the following are speeds at equal steps until the last one is the speed at the
   * end of the transmit range ( {@link NetworkInterface#TRANSMIT_RANGE_S}). The speed between the
   * steps is linearly interpolated.
   */
  ...
  /**
   * Tries to connect this host to another host. The other host must be active and within range of
  ...
  public void connect(NetworkInterface anotherInterface) {
    if (isScanning()
      ...
      Connection con =
          new VBRConnection(this.host, this, anotherInterface.getHost(), anotherInterface);
  ...
  /**
   * Returns the transmit speed to another interface based on the distance to this interface
   *
  ...
  public int getTransmitSpeed(NetworkInterface ni) {
    ...
    fractionIndex = (distance / this.transmitRange) * (this.transmitSpeeds.length - 1);
    index = (int) (fractionIndex);
    decimal = fractionIndex - index;

    speed = this.transmitSpeeds[index] * (1 - decimal) + this.transmitSpeeds[index + 1] * decimal;
