...
/**
 * Overlay grid of the world where each interface is put on a cell depending of its location. This
 * is used in cell-based optimization of connecting the interfaces.
 *
 * <p>The idea in short:<br>
 * Instead of checking for every interface if some of the other interfaces are close enough (this
 * approach obviously doesn't scale) we check only interfaces that are "close enough" to be possibly
 * connected. Being close enough is determined by keeping track of the approximate location of the
 * interfaces by storing them in overlay grid's cells and updating the cell information every time
 * the interfaces move. If two interfaces are in the same cell or in neighboring cells, they have a
 * chance of being close enough for connection. Then only that subset of interfaces is checked for
 * possible connectivity.
 *
 * <p><strong>Note:</strong> this class does NOT support negative coordinates. Also, it makes sense
 * to normalize the coordinates to start from zero to conserve memory.
 */
public class ConnectivityGrid extends ConnectivityOptimizer {

  /**
   * Cell based optimization cell size multiplier -setting id ({@value}). Used in {@link
   * World#OPTIMIZATION_SETTINGS_NS} name space. Single ConnectivityCell's size is the biggest radio
   * range times this. Larger values save memory and decrease startup time but may result in slower
   * simulation. Default value is {@link #DEF_CON_CELL_SIZE_MULT}. Smallest accepted value is 1.
   */
   ...
   *
   * @param cellSize Cell's edge's length (must be larger than the largest radio coverage's
   *     diameter)
   */
    ...
    if (cellSizeMultiplier < 1) {
      throw new SettingsError(
          "Too small value ("
              + cellSizeMultiplier
              + ") for "
              + World.OPTIMIZATION_SETTINGS_NS
              + "."
              + CELL_SIZE_MULT_S);
    }
  }
   ...
   * @param maxRange Maximum range used by the radio technology using this connectivity grid.
  ...
  public static ConnectivityGrid ConnectivityGridFactory(int key, double maxRange) {
    if (gridobjects.containsKey(key)) {
      ...
      ConnectivityGrid newgrid =
          new ConnectivityGrid((int) Math.ceil(maxRange * cellSizeMultiplier));
  ...
  /**
   * Returns an array of Cells that contains the neighbors of a certain cell and the cell itself.
   *
  ...
  /**
   * Returns all interfaces that are "near" (i.e., in neighboring grid cells) and use the same
   * technology and channel as the given interface
   *
  ...
  /**
   * A single cell in the cell grid. Contains the interfaces that are currently in that part of the
   * grid.
   */
  public class GridCell {
    ...
    private GridCell() {
      this.interfaces = new ArrayList<NetworkInterface>(EXPECTED_INTERFACE_COUNT);
    }
    ...
    public void moveInterface(NetworkInterface ni, GridCell to) {
      ...
      assert removeOk : "interface " + ni + " not found from cell with " + interfaces;
    }
