...
/**
 * A simple Network Interface that provides a variable bit-rate service, where the bit-rate depends
 * on the number of other transmitting stations within range The current transmit speed is updated
 * only if there are ongoing transmissions. The configured transmit speed is the maximum obtainable
 * speed.
 */
public class InterferenceLimitedInterface extends NetworkInterface {
  ...
  public void update() {
    ...
    for (NetworkInterface i : interfaces) connect(i);
    ...
    for (Connection con : this.connections) {
      ...
      if (con.getOtherInterface(this).isTransferring() == true) {
        numberOfActive++;
      }
    }
        ...
        (int)
            Math.floor(
                (double) transmitSpeed
                    / (Math.sqrt((1.0 * numberOfActive) * Math.log(1.0 * numberOfActive)))
                    / ntrans);
