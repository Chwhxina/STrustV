...
public class MessageDeleteEvent extends MessageEvent {
  ...
  public void processEvent(World world) {
    ...
    if (this.id.equals(StandardEventsReader.ALL_MESSAGES_ID)) {
      List<String> ids = new ArrayList<>();
      for (Message m : host.getMessageCollection()) {
        ids.add(m.getId());
      }
      for (String nextId : ids) {
        host.deleteMessage(nextId, this.drop);
      }
    } else {
      host.deleteMessage(this.id, this.drop);
    }
  }

  @Override
  public String toString() {
    return super.toString() + " [" + this.fromAddr + "] DELETE";
  }
}
