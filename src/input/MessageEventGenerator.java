...
public class MessageEventGenerator implements EventQueue {
   ...
   * of uniformly distributed random values. Defines the message size (bytes).
   */
   ...
   * {@link #HOST_RANGE_S} setting's range. The lower bound is inclusive and upper bound exclusive.
   */
   ...
   * messages are created for the whole simulation time.
   */
  ...
  public MessageEventGenerator(Settings s) {
    this.sizeRange = s.getCsvInts(MessageEventGenerator.MESSAGE_SIZE_S);
    this.msgInterval = s.getCsvInts(MessageEventGenerator.MESSAGE_INTERVAL_S);
    this.hostRange = s.getCsvInts(MessageEventGenerator.HOST_RANGE_S, 2);
    this.idPrefix = s.getSetting(MessageEventGenerator.MESSAGE_ID_PREFIX_S);

    if (s.contains(MessageEventGenerator.MESSAGE_TIME_S)) {
      this.msgTime = s.getCsvDoubles(MessageEventGenerator.MESSAGE_TIME_S, 2);
    } else {
      this.msgTime = null;
    }
    if (s.contains(MessageEventGenerator.TO_HOST_RANGE_S)) {
      this.toHostRange = s.getCsvInts(MessageEventGenerator.TO_HOST_RANGE_S, 2);
    } else {
      this.toHostRange = null;
    }

    /* if prefix is unique, so will be the rng's sequence */
    this.rng = new Random(this.idPrefix.hashCode());
    ...
    if (this.sizeRange.length == 1) {
      ...
      s.assertValidRange(this.sizeRange, MessageEventGenerator.MESSAGE_SIZE_S);
    }
    if (this.msgInterval.length == 1) {
      this.msgInterval = new int[] {this.msgInterval[0], this.msgInterval[0]};
    } else {
      s.assertValidRange(this.msgInterval, MessageEventGenerator.MESSAGE_INTERVAL_S);
    }
    s.assertValidRange(this.hostRange, MessageEventGenerator.HOST_RANGE_S);
    ...
    if (this.hostRange[1] - this.hostRange[0] < 2) {
      ...
      } else if (this.toHostRange[0] == this.hostRange[0] && this.toHostRange[1] == this.hostRange[1]) {
    ...
    this.nextEventsTime =
        (this.msgTime != null ? this.msgTime[0] : 0)
            + this.msgInterval[0]
            + (this.msgInterval[0] == this.msgInterval[1] ? 0 : this.rng.nextInt(
            this.msgInterval[1] - this.msgInterval[0]));
  }
  ...
  protected int drawHostAddress(int[] hostRange) {
    ...
    return hostRange[0] + this.rng.nextInt(hostRange[1] - hostRange[0]);
  }
  ...
  protected int drawMessageSize() {
    int sizeDiff = this.sizeRange[0] == this.sizeRange[1] ? 0 : this.rng.nextInt(
        this.sizeRange[1] - this.sizeRange[0]);
    return this.sizeRange[0] + sizeDiff;
  }
  ...
  protected int drawNextEventTimeDiff() {
    int timeDiff =
        this.msgInterval[0] == this.msgInterval[1] ? 0 : this.rng.nextInt(
            this.msgInterval[1] - this.msgInterval[0]);
    return this.msgInterval[0] + timeDiff;
  }
  ...
  protected int drawToAddress(int[] hostRange, int from) {
    ...
    do {
      to =
          this.toHostRange != null
              ? this.drawHostAddress(this.toHostRange)
              : this.drawHostAddress(this.hostRange);
    } while (from == to);
  ...
  public ExternalEvent nextEvent() {
    ...
    to = this.drawToAddress(this.hostRange, from);

    msgSize = this.drawMessageSize();
    interval = this.drawNextEventTimeDiff();
