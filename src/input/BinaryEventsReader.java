...
/**
 * Reads External Events from a binary file. Can also create binary files from a list of external
 * events.
 */
public class BinaryEventsReader implements ExternalEventsReader {
  ...
  public BinaryEventsReader(File eventsFile) {
    try {
      ...
      this.in = new ObjectInputStream(fis);
      // first object should tell the amount of events
      this.eventsLeft = (Integer) this.in.readObject();
    } catch (IOException e) {
      throw new SimError(e);
    } catch (ClassNotFoundException e) {
      throw new SimError(
          "Invalid binary input file for external " + "events:" + eventsFile.getAbsolutePath(), e);
    }
  }
  ...
  public static boolean isBinaryEeFile(File file) {
    if (!file.getName().endsWith(BinaryEventsReader.BINARY_EXT)) {
      return false;
    }
   ...
   */
  public static void storeToBinaryFile(String fileName, List<ExternalEvent> events)
      throws IOException {

    // make sure the file name ends with binary extension
    if (!fileName.endsWith(BinaryEventsReader.BINARY_EXT)) {
      fileName += "BINARY_EXT";
    }
  ...
  public List<ExternalEvent> readEvents(int nrof) {
    ...
    try {
      for (int i = 0; i < nrof && this.eventsLeft > 0; i++) {
        events.add((ExternalEvent) this.in.readObject());
        this.eventsLeft--;
      }
      if (this.eventsLeft == 0) {
        this.in.close();
