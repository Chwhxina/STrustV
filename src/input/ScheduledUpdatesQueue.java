...
/**
 * Event queue where simulation objects can request an update to happen at the specified simulation
 * time. Multiple updates at the same time are merged to a single update.
 */
public class ScheduledUpdatesQueue implements EventQueue {
  ...
  public void addUpdate(double simTime) {
    ...
    if (ee.compareTo(this.nextEvent) == 0) { // this event is already next
      return;
    } else if (this.nextEvent.getTime() > simTime) { // new nextEvent
      this.putToQueue(this.nextEvent); // put the old nextEvent back to q
      this.nextEvent = ee;
  ...
  private void putToQueue(ExternalEvent ee) {
    ...
    for (int i = 0, n = this.updates.size(); i < n; i++) {
      double time = this.updates.get(i).getTime();
      if (time == eeTime) {
