...
public class ExternalPathMovementReader {
  ...
  /**
   * Creates a new reader by parsing the given files and building the internal caches.
   *
   * @param traceFilePath path to the trace file
   * @param activityFilePath path to the activity file
   */
  private ExternalPathMovementReader(String traceFilePath, String activityFilePath)
      throws IOException {
    ...
    try {
      if (traceFilePath.endsWith(".zip")) {
        ...
        reader = new BufferedReader(new FileReader(traceFilePath));
      }
    } catch (FileNotFoundException e1) {
      throw new SettingsError("Couldn't find external movement input " + "file " + inFile);
    } finally {
      if (zf != null) {
        zf.close();
      }
    ...
    while (line != null) {
      ...
      List<Entry> path = new LinkedList<>();
    ...
    try {
      if (activityFilePath.endsWith(".zip")) {
        ...
        reader = new BufferedReader(new FileReader(activityFilePath));
      }
    } catch (FileNotFoundException e) {
      throw new SettingsError("Couldn't find external activity input " + "file " + inFile);
    } finally {
      if (zf != null) {
        zf.close();
      }
  ...
  /**
   * Get an instance of the reader for the given file path. If the file has already been read
   * previously it will not be read again and instead the previous instance of the reader will be
   * returned.
   *
   * @param traceFilePath path where the trace file is read from
   * @param activeFilePath path where the activity file is read from
   * @return instance of the reader that has loaded all the paths from the given trace file.
   */
  public static ExternalPathMovementReader getInstance(
      String traceFilePath, String activeFilePath) {
    if (!ExternalPathMovementReader.singletons.containsKey(traceFilePath)) {
      try {
        ExternalPathMovementReader.singletons.put(
            traceFilePath, new ExternalPathMovementReader(traceFilePath, activeFilePath));
      } catch (IOException e) {
        System.exit(1);
      }
    }
  ...
  /**
   * Sets normalizing of read values on/off. If on, values returned by readNextMovements() are
   * decremented by minimum values of the offsets. Default is on (normalize).
   *
