...
/**
 * Manages the external convergence layer connections to dtnd. Parses the configuration file and
 * sets up the CLAParsers and EID->host mappings.
 *
 * @author teemuk
 */
public class DTN2Manager {
  ...
  /**
   * Sets up the dtnd connections by parsing the configuration file defined in the <code>
   * DTN2.configFile</code> setting.
   *
  ...
  public static void setup(World world) {
    ...
    try {
      ...
      if (DEBUG) {
        Debug.p("Could not load requested DTN2 configuration " + "file '" + fname + "'.");
      }
      return;
    }
    ...
    while (s != null) {
      ...
      if (attrs.length == 5 && !s.startsWith("#")) {
        ...
        DTN2Manager.EIDHost e = new DTN2Manager.EIDHost(nodeEID, nodeID, h);
        ...
        DTN2Events.ParserHandler ph =
            DTN2Manager.events.getParserHandler(nodeID, dtnd_host, console_port);
        p.setListener(ph);
  ...
  /**
   * Sets the <code>DTN2Reporter</code> object used to pass messages from ONE to dtnd. This should
   * be used by the dynamically loaded DTN2Reporter object to allow other objects get reference to
   * it.
   *
  ...
  /**
   * Returns a <code>Collection</code> of <code>DTNHost</code> objects corresponding to the given
   * EID.
   *
  ...
  public static Collection<EIDHost> getHosts(String EID) {
    Collection<EIDHost> c = new LinkedList<>();
