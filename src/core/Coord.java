...
public class Coord implements Cloneable, Comparable<Coord> {
  ...
  public boolean equals(Coord c) {
    if (c == this) {
      ...
      return (this.x == c.x && this.y == c.y); // XXX: == for doubles...
    }
  }
  ...
  /**
   * Compares this coordinate to other coordinate. Coordinate whose y value is smaller comes first
   * and if y values are equal, the one with smaller x value comes first.
   *
   * @return -1, 0 or 1 if this node is before, in the same place or after the other coordinate
   */
  @Override
