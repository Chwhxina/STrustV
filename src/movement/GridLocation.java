...
public class GridLocation extends MovementModel {
  ...
  /**
   * How many rows of nodes there are -setting ({@value}). Number of columns is calculated from the
   * node count.
   */
  public static final String ROWS_S = "rows";
  /** Space between the nodes -setting ({@value}) */
  public static final String SPACING_S = "spacing";
  /** Maximum random offset for the location of the nodes -setting ({@value}). Default = 0. */
  public static final String OFFSET_S = "randomOffset";
  /** Location of the first node (grid's upper left corner) -setting ({@value}). Two values: x,y */
  ...
  public GridLocation(Settings s) {
    ...
    s.setSubNameSpace(GridLocation.GRIDLOC_NS);

    this.rows = s.getInt(GridLocation.ROWS_S);
    this.cols = (nodeCount / this.rows);
    this.startCoords = s.getCsvDoubles(GridLocation.LOCATION_S, 2);
    this.spacing = s.getInt(GridLocation.SPACING_S);
    this.offset = s.getDouble(GridLocation.OFFSET_S, 0);
  ...
  public GridLocation(GridLocation proto) {
    ...
    x = proto.startCoords[0] + ((proto.nodeCount) % proto.cols) * proto.spacing;
    x += MovementModel.rng.nextDouble() * proto.offset;

    y = proto.startCoords[1] + ((proto.nodeCount) / proto.cols) * proto.spacing;
    y += MovementModel.rng.nextDouble() * proto.offset;
