...
/**
 * This class controls busses and passengers that can use the bus. There can be many bus
 * BusControlSystems, but a bus or passenger can only belong to one system.
 *
 * @author Frans Ekman
 */
public class BusControlSystem {
  ...
  public static void reset() {
    BusControlSystem.systems = new HashMap<>();
  }

  /**
   * Returns a reference to a BusControlSystem with ID provided as parameter. If a system does not
   * already exist with the requested ID, a new one is created.
   *
  ...
  public static BusControlSystem getBusControlSystem(int systemID) {
    ...
    if (BusControlSystem.systems.containsKey(id)) {
      return BusControlSystem.systems.get(id);
    } else {
      BusControlSystem bcs = new BusControlSystem(systemID);
      BusControlSystem.systems.put(id, bcs);
      return bcs;
    }
  }
  ...
  public void busHasStopped(int busID, Coord busStop, Path nextPath) {
    Iterator<BusTravellerMovement> iterator = this.travellers.values().iterator();
    while (iterator.hasNext()) {
      BusTravellerMovement traveller = iterator.next();
  ...
  public void registerTraveller(BusTravellerMovement traveller) {
    this.travellers.put(traveller.getID(), traveller);
  }
