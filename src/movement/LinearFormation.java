...
/**
 * A stationary "movement" model where nodes do not move but are in linear formation (i.e., in a
 * line).
 */
public class LinearFormation extends MovementModel {
  ...
  public LinearFormation(Settings s) {
    ...
    coords = s.getCsvInts(LinearFormation.LINEAR_FORMATION_NS + LinearFormation.START_LOCATION_S, 2);
    this.startLoc = new Coord(coords[0], coords[1]);
    coords = s.getCsvInts(LinearFormation.LINEAR_FORMATION_NS + LinearFormation.END_LOCATION_S, 2);
  ...
  public LinearFormation(LinearFormation lf) {
    super(lf);
    this.loc = this.calculateInitLocation(lf);
  }
  ...
  private Coord calculateInitLocation(LinearFormation proto) {
    ...
    dy = placementFraction * (proto.endLoc.getY() - proto.startLoc.getY());
    c.translate(dx, dy);
