...
/**
 * A node in a SimMap. Node has a location, 0-n neighbors that it is connected to and possibly a
 * type identifier.
 */
public class MapNode implements Comparable<MapNode> {
  ...
  public void addType(int type) {
    this.type |= this.typeToBitMask(type);
  }

  /**
   * Returns true if this node is of given type, false if none of node's type(s) match to given type
   * or node doesn't have type at all
   *
   * @param type The type (integer from range [{@value MIN_TYPE}, {@value MAX_TYPE}])
  ...
  public boolean isType(int type) {
    ...
    return (this.type & this.typeToBitMask(type)) != 0;
  }

  /**
   * Returns true if the node's types match any of the given types
   *
   * @param types The types to check (array of values in range [{@value MIN_TYPE}, {@value
   *     MAX_TYPE}])
   * @return True if at least one of the types matched, false if none of the types matched
   * @see #isType(int)
   */
  ...
  private int typeToBitMask(int type) {
    assert type >= MapNode.MIN_TYPE && type <= MapNode.MAX_TYPE : "Invalid node type " + type;
  ...
  public void addNeighbor(MapNode node) {
    ...
    this.addToList(node); // add the node to list
  }
  ...
  public String toString() {
    return "N" + (this.type != 0 ? "t" + this.type : "") + "@" + this.location.toString();
  }
