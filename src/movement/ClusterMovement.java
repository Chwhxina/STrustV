...
/**
 * Random waypoint movement where the coordinates are restricted to circular area defined by a
 * central point and range.
 *
 * @author teemuk
 */
package movement;
...
public class ClusterMovement extends RandomWaypoint {
  ...
  public ClusterMovement(Settings s) {
    ...
    if (s.contains(ClusterMovement.CLUSTER_RANGE)) {
      this.p_range = s.getDouble(ClusterMovement.CLUSTER_RANGE);
    }
    if (s.contains(ClusterMovement.CLUSTER_CENTER)) {
      int[] center = s.getCsvInts(ClusterMovement.CLUSTER_CENTER, 2);
  ...
  protected Coord randomCoord() {
    double x = (MovementModel.rng.nextDouble() * 2 - 1) * this.p_range;
    double y = (MovementModel.rng.nextDouble() * 2 - 1) * this.p_range;
    while (x * x + y * y > this.p_range * this.p_range) {
      x = (MovementModel.rng.nextDouble() * 2 - 1) * this.p_range;
      y = (MovementModel.rng.nextDouble() * 2 - 1) * this.p_range;
    }
    x += this.p_x_center;
    y += this.p_y_center;
