...
/**
 * External movement trace reader for traces that are in path format. See <code>
 * ExternalPathMovementReader</code> for details.
 *
 * @author teemuk
 */
public class ExternalPathMovement extends MovementModel {
  ...
  public ExternalPathMovement(Settings settings) {
    this.traceFile = settings.getSetting(ExternalPathMovement.MOVEMENT_FILE_S);
    this.activeFile = settings.getSetting(ExternalPathMovement.ACTIVITY_FILE_S);
  }
  ...
  /** Initializes the movement model. Uses a reader to get the paths for this host. */
  private void init() {
    // Get paths for this node
    ExternalPathMovementReader reader =
        ExternalPathMovementReader.getInstance(this.traceFile, this.activeFile);
    this.paths = reader.getPaths(this.getHost().getAddress());
    this.active = reader.getActive(this.getHost().getAddress());
  }
  ...
  public Path getPath() {
    ...
    // Get the path
    List<ExternalPathMovementReader.Entry> path = this.paths.get(this.curPath);
    this.curPath++;
    ...
    if (curPos.getX() != pathStart.x || curPos.getY() != pathStart.y) {
    ...
    for (int i = 1; i < path.size(); i++) {
      ...
      double ds = Math.sqrt((e.x - e2.x) * (e.x - e2.x) + (e.y - e2.y) * (e.y - e2.y));
      double v = ds / dt;
      p.addWaypoint(c, v);
    }
  ...
  public Coord getInitialLocation() {
    ...
    if (this.paths.size() > 0 && this.paths.get(0).size() > 0) {
