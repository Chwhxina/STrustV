...
/** Epidemic-like message router making waves of messages. Work in progress. */
public class WaveRouter extends ActiveRouter {

  /**
   * Immunity time -setting id ({@value}). Defines how long time a node will reject incoming
   * messages it has already received
   */
  public static final String IMMUNITY_S = "immunityTime";
  /**
   * Custody fraction -setting id ({@value}). Defines how long (compared to immunity time) nodes
   * accept custody for new incoming messages.
   */
  ...
  public WaveRouter(Settings s) {
    super(s);
    this.immunityTime = s.getDouble(WaveRouter.IMMUNITY_S);
    this.custodyFraction = s.getDouble(WaveRouter.CUSTODY_S);
  }
  ...
  protected int checkReceiving(Message m, DTNHost from) {
    ...
    if (lastTime != null) {
      if (lastTime + this.immunityTime > SimClock.getTime()) {
        return MessageRouter.DENIED_POLICY; /* still immune to the message */
      } else {
  ...
  protected Message getNextMessageToRemove(boolean excludeMsgBeingSent) {
    ...
    for (Message m : messages) {
      ...
      if (custodyStartTime != null) {
        if (SimClock.getTime() > custodyStartTime + this.immunityTime * this.custodyFraction) {
  ...
  public RoutingInfo getRoutingInfo() {
    ...
    RoutingInfo immunity = new RoutingInfo("Immune to " + this.recentMessages.size() + " messages");

    for (String id : this.recentMessages.keySet()) {
      RoutingInfo m =
          new RoutingInfo(
              id + " until " + String.format("%.2f", this.recentMessages.get(id) + this.immunityTime));
