...
/**
 * Implementation of MaxProp router as described in <I>MaxProp: Routing for Vehicle-Based
 * Disruption-Tolerant Networks</I> by John Burgess et al.
 *
 * <p>but with parameter estimation for finding an alpha based on timescale definition:
 *
 * <p>Extension of the protocol by adding a parameter alpha (default 1) By new connection, the
 * delivery likelihood is increased by alpha and divided by 1+alpha. Using the default results in
 * the original algorithm. Refer to Karvo and Ott, <I>Time Scales and Delay-Tolerant Routing
 * Protocols</I> Chants, 2008
 *
 * <p>This version tries to estimate a good value of alpha from a timescale parameter given by the
 * user, and from the encounters the node sees during simulation.
 *
 * @version 1.0
 */
public class MaxPropRouterWithEstimation extends ActiveRouter {
  ...
  /**
   * Over how many samples the "average number of bytes transferred per transfer opportunity" is
   * taken
   */
  ...
  /**
   * meeting probabilities of all hosts from this host's point of view mapped using host's network
   * address
   */
  ...
  /**
   * mapping of the current costs for all messages. This should be set to null always when the costs
   * should be updated (a host is met or a new message is received)
   */
  ...
  public MaxPropRouterWithEstimation(Settings settings) {
    super(settings);
    Settings maxPropSettings = new Settings(MaxPropRouterWithEstimation.MAXPROP_NS);
    this.alpha = MaxPropRouterWithEstimation.DEFAULT_ALPHA;
    this.timescale = maxPropSettings.getInt(MaxPropRouterWithEstimation.TIME_SCALE_S);
    this.initMeetings();
  }
  ...
  protected MaxPropRouterWithEstimation(MaxPropRouterWithEstimation r) {
    ...
    this.probs = new MeetingProbabilitySet(MeetingProbabilitySet.INFINITE_SET_SIZE, this.alpha);
    this.allProbs = new HashMap<>();
    this.dijkstra = new MaxPropDijkstra(this.allProbs);
    this.ackedMessageIds = new HashSet<>();
    this.avgSamples = new int[MaxPropRouterWithEstimation.BYTES_TRANSFERRED_AVG_SAMPLES];
    this.initMeetings();
  }
  ...
  public void changedConnection(Connection con) {
    ...
    if (con.isUp()) { // new connection
      ...
      if (con.isInitiator(this.getHost())) {
        ...
        assert mRouter instanceof MaxPropRouterWithEstimation
            : "MaxProp only works " + " with other routers of same type";
        ...
        if (otherRouter.updateEstimators(this.getHost())) {
        ...
        this.probs.updateMeetingProbFor(otherHost.getAddress());
        otherRouter.probs.updateMeetingProbFor(this.getHost().getAddress());
        ...
        otherRouter.allProbs.put(this.getHost().getAddress(), this.probs.replicate());
      }
    } else {
      /* connection went down, update transferred bytes average */
      this.updateTransferredBytesAvg(con.getTotalBytesTransferred());
    }
  }

  /**
   * Updates transitive probability values by replacing the current MeetingProbabilitySets with the
   * values from the given mapping if the given sets have more recent updates.
   *
  ...
  private void updateTransitiveProbs(Map<Integer, MeetingProbabilitySet> p) {
    for (Map.Entry<Integer, MeetingProbabilitySet> e : p.entrySet()) {
      ...
      if (myMps == null || e.getValue().getLastUpdateTime() > myMps.getLastUpdateTime()) {
  ...
  protected boolean updateEstimators(DTNHost host) {
    ...
    if (this.meetings.containsKey(host)) {
      double timeDiff = currentTime - this.meetings.get(host);
      // System.out.printf("current time: %f\t last time:  %f\n",currentTime,meetings.get(host));

      this.nrofSamplesIET++;
      this.meanIET =
          (((double) this.nrofSamplesIET - 1) / (double) this.nrofSamplesIET) * this.meanIET
              + (1 / (double) this.nrofSamplesIET) * timeDiff;
      this.meetings.put(host, currentTime);
    } else {
      /* nothing to update */
      this.meetings.put(host, currentTime);
    }
     ...
     *
     */
    this.nrofTotENC++; // the number of encounter
    if (this.encounters.containsKey(host)) {
      int encounterNro = this.nrofTotENC - this.encounters.get(host);
      // System.out.printf("current time: %f\t last time:  %f\n",currentTime,meetings.get(host));

      this.nrofSamplesENC++;
      this.meanENC =
          (((double) this.nrofSamplesENC - 1) / (double) this.nrofSamplesENC) * this.meanENC
              + (1 / (double) this.nrofSamplesENC) * (double) encounterNro;
      this.encounters.put(host, this.nrofTotENC);
      return true;
    } else {
      /* nothing to update */
      this.encounters.put(host, this.nrofTotENC);
  ...
  protected void updateParam() {
    double err = .01;
    double ntarg = Math.ceil(this.timescale / this.meanIET);
    double ee = 1;
    double alphadiff = .1;
    int ob = 0;
    double fstable;
     ...
     */
    if (this.meanIET > (double) this.timescale) {
      System.out.printf("meanIET %f > %d timescale\n", this.meanIET, this.timescale);
      return;
    }
    ...
    while (ee != err) {
      A = Math.pow(1 + this.alpha, this.meanENC + 1);
      fstable = this.alpha / (A - 1);
      fnzero = (this.alpha / A) * (1 - Math.pow(A, -ntarg)) / (1 - 1 / A);
      ...
      if (ee > err) {
        ...
        ob = 1;
        this.alpha = this.alpha + alphadiff;
      } else {
        if (ee < (err - err * 0.001)) {
          if (ob == 1) {
            alphadiff = alphadiff / 2.0;
    ...
    this.probs.setAlpha(this.alpha);
  }
  ...
  /**
   * Method is called just before a transfer is finalized at {@link ActiveRouter#update()}. MaxProp
   * makes book keeping of the delivered messages so their IDs are stored.
   *
  ...
  private void updateTransferredBytesAvg(int newValue) {
    ...
    if (this.nextSampleIndex >= MaxPropRouterWithEstimation.BYTES_TRANSFERRED_AVG_SAMPLES) {
      this.nextSampleIndex = 0;
    }

    for (int i = 0; i < MaxPropRouterWithEstimation.BYTES_TRANSFERRED_AVG_SAMPLES; i++) {
  ...
  /**
   * Returns the next message that should be dropped, according to MaxProp's message ordering scheme
   * (see MaxPropTupleComparator).
   *
   * @param excludeMsgBeingSent If true, excludes message(s) that are being sent from the
   *     next-to-be-dropped check (i.e., if next message to drop is being sent, the following
   *     message is returned)
  ...
  /**
   * Returns the message delivery cost between two hosts from this host's point of view. If there is
   * no path between "from" and "to" host, Double.MAX_VALUE is returned. Paths are calculated only
   * to hosts that this host has messages to.
   *
   * @param from The host where a message is coming from
   * @param to The host where a message would be destined to
   * @return The cost of the cheapest path to the destination or Double.MAX_VALUE if such a path
   *     doesn't exist
   */
  public double getCost(DTNHost from, DTNHost to) {
    /* check if the cached values are OK */
    if (this.costsForMessages == null || this.lastCostFrom != from) {
      /* cached costs are invalid -> calculate new costs */
      this.allProbs.put(this.getHost().getAddress(), this.probs);
      ...
      this.costsForMessages = this.dijkstra.getCosts(fromIndex, toSet);
      this.lastCostFrom = from; // store source host for caching checks
    }

    if (this.costsForMessages.containsKey(to.getAddress())) {
      return this.costsForMessages.get(to.getAddress());
    } else {
  ...
  /**
   * Tries to send all other messages to all connected hosts ordered by hop counts and their
  ...
  private Tuple<Message, Connection> tryOtherMessages() {
    ...
    Collections.sort(messages, new MaxPropTupleComparator(this.calcThreshold()));
    return this.tryMessagesForConnected(messages);
  }

  /**
   * Calculates and returns the current threshold value for the buffer's split based on the average
   * number of bytes transferred per transfer opportunity and the hop counts of the messages in the
   * buffer. Method is public only to make testing easier.
   *
  ...
  public RoutingInfo getRoutingInfo() {
    ...
    RoutingInfo ri = new RoutingInfo(this.probs.getAllProbs().size() + " meeting probabilities");

    /* show meeting probabilities for this host */
    for (Map.Entry<Integer, Double> e : this.probs.getAllProbs().entrySet()) {
      Integer host = e.getKey();
      Double value = e.getValue();
      ri.addMoreInfo(new RoutingInfo(String.format("host %d : %.6f", host, value)));
    }

    ri.addMoreInfo(
        new RoutingInfo(String.format("meanIET: %f\t from %d samples", this.meanIET,
            this.nrofSamplesIET)));
    ri.addMoreInfo(
        new RoutingInfo(String.format("meanENC: %f\t from %d samples", this.meanENC,
            this.nrofSamplesENC)));
    ri.addMoreInfo(new RoutingInfo(String.format("current alpha: %f", this.alpha)));

    top.addMoreInfo(ri);
    top.addMoreInfo(new RoutingInfo("Avg transferred bytes: " + this.avgTransferredBytes));
  ...
  /**
   * Message comparator for the MaxProp routing module. Messages that have a hop count smaller than
   * the given threshold are given priority and they are ordered by their hop count. Other messages
   * are ordered by their delivery cost.
   */
  private class MaxPropComparator implements Comparator<Message> {
    ...
    /**
     * Constructor. Assumes that the host where all the costs are calculated from is this router's
     * host.
     *
     * @param treshold Messages with the hop count smaller than this value are transferred first
     *     (and ordered by the hop count)
     */
    public MaxPropComparator(int treshold) {
      this.threshold = treshold;
      this.from1 = this.from2 = MaxPropRouterWithEstimation.this.getHost();
    }
    ...
    /**
     * Compares two messages and returns -1 if the first given message should be first in order, 1
     * if the second message should be first or 0 if message order can't be decided. If both
     * messages' hop count is less than the threshold, messages are compared by their hop count
     * (smaller is first). If only other's hop count is below the threshold, that comes first. If
     * both messages are below the threshold, the one with smaller cost (determined by {@link
     * MaxPropRouterWithEstimation#getCost(DTNHost, DTNHost)}) is first.
     */
    @Override
    public int compare(Message msg1, Message msg2) {
      ...
      if (hopc1 < this.threshold && hopc2 >= this.threshold) {
        return -1; // message1 should be first
      } else if (hopc2 < this.threshold && hopc1 >= this.threshold) {
        return 1; // message2 -"-
      }
      ...
      p1 = MaxPropRouterWithEstimation.this.getCost(this.from1, msg1.getTo());
      p2 = MaxPropRouterWithEstimation.this.getCost(this.from2, msg2.getTo());
      ...
      if (p1 - p2 == 0) {
        ...
        if (hopc1 == hopc2) {
          return MaxPropRouterWithEstimation.this.compareByQueueMode(msg1, msg2);
        } else {
          return hopc1 - hopc2;
        }
      } else if (p1 - p2 < 0) {
  ...
  /**
   * Message-Connection tuple comparator for the MaxProp routing module. Uses MaxPropComparator on
   * the messages of the tuples setting the "from" host for that message to be the one in the
   * connection tuple (i.e., path is calculated starting from the host on the other end of the
   * connection).
   */
  private class MaxPropTupleComparator implements Comparator<Tuple<Message, Connection>> {
    private final int threshold;
    ...
    /**
     * Compares two message-connection tuples using the {@link MaxPropComparator#compare(Message,
     * Message)}.
     */
    @Override
    public int compare(Tuple<Message, Connection> tuple1, Tuple<Message, Connection> tuple2) {
      MaxPropComparator comp;
      DTNHost from1 = tuple1.getValue().getOtherNode(MaxPropRouterWithEstimation.this.getHost());
      DTNHost from2 = tuple2.getValue().getOtherNode(MaxPropRouterWithEstimation.this.getHost());

      comp = new MaxPropComparator(this.threshold, from1, from2);
