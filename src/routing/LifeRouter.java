...
public class LifeRouter extends ActiveRouter {
  ...
  /**
   * Neighboring message count -setting id ({@value}). Two comma separated values: min and max. Only
   * if the amount of connected nodes with the given message is between the min and max value, the
   * message is accepted for transfer and kept in the buffer.
   */
  ...
  public LifeRouter(Settings s) {
    super(s);
    this.countRange = s.getCsvInts(LifeRouter.NM_COUNT_S, 2);
  }
  ...
  protected int checkReceiving(Message m, DTNHost from) {
    int peerMsgCount = this.getPeerMessageCount(m);
  ...
  public void update() {
    ...
    for (Message m : this.getMessageCollection()) {
      peerMsgCount = this.getPeerMessageCount(m);
      if (peerMsgCount < this.countRange[0] || peerMsgCount > this.countRange[1]) {
