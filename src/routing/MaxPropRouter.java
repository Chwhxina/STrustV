 ...
 *     delivery likelihood is increased by alpha and divided by 1+alpha. Using the default results
 *     in the original algorithm. Refer to Karvo and Ott, <I>Time Scales and Delay-Tolerant Routing
...
public class MaxPropRouter extends ActiveRouter {
  ...
  /**
   * Meeting probability set maximum size -setting id ({@value}). The maximum amount of meeting
   * probabilities to store.
   */
  ...
  public MaxPropRouter(Settings settings) {
    super(settings);
    Settings maxPropSettings = new Settings(MaxPropRouter.MAXPROP_NS);
    if (maxPropSettings.contains(MaxPropRouter.ALPHA_S)) {
      this.alpha = maxPropSettings.getDouble(MaxPropRouter.ALPHA_S);
    } else {
      this.alpha = MaxPropRouter.DEFAULT_ALPHA;
    }

    Settings mpSettings = new Settings(MaxPropRouter.MAXPROP_NS);
    if (mpSettings.contains(MaxPropRouter.PROB_SET_MAX_SIZE_S)) {
      MaxPropRouter.probSetMaxSize = mpSettings.getInt(MaxPropRouter.PROB_SET_MAX_SIZE_S);
    } else {
      MaxPropRouter.probSetMaxSize = MaxPropRouter.DEFAULT_PROB_SET_MAX_SIZE;
    }
  }
  ...
  protected MaxPropRouter(MaxPropRouter r) {
    super(r);
    this.alpha = r.alpha;
    this.probs = new MeetingProbabilitySet(MaxPropRouter.probSetMaxSize, this.alpha);
    this.allProbs = new HashMap<>();
    this.dijkstra = new MaxPropDijkstra(this.allProbs);
    this.ackedMessageIds = new HashSet<>();
    this.avgSamples = new int[MaxPropRouter.BYTES_TRANSFERRED_AVG_SAMPLES];
  ...
  public void changedConnection(Connection con) {
    ...
    if (con.isUp()) { // new connection
      ...
      if (con.isInitiator(this.getHost())) {
        ...
        assert mRouter instanceof MaxPropRouter
  ...
  protected void transferDone(Connection con) {
    ...
    String id = m.getId();
    DTNHost recipient = con.getOtherNode(this.getHost());
  ...
  private void updateTransferredBytesAvg(int newValue) {
    ...
    if (this.nextSampleIndex >= MaxPropRouter.BYTES_TRANSFERRED_AVG_SAMPLES) {
      this.nextSampleIndex = 0;
    }

    for (int i = 0; i < MaxPropRouter.BYTES_TRANSFERRED_AVG_SAMPLES; i++) {
  ...
  private class MaxPropComparator implements Comparator<Message> {
    ...
    public MaxPropComparator(int treshold) {
      ...
      this.from1 = this.from2 = MaxPropRouter.this.getHost();
    }
     ...
     * MaxPropRouter#getCost(DTNHost, DTNHost)}) is first.
     */
    @Override
    public int compare(Message msg1, Message msg2) {
      ...
      p1 = MaxPropRouter.this.getCost(this.from1, msg1.getTo());
      p2 = MaxPropRouter.this.getCost(this.from2, msg2.getTo());
      ...
      if (p1 - p2 == 0) {
        ...
        if (hopc1 == hopc2) {
          return MaxPropRouter.this.compareByQueueMode(msg1, msg2);
        } else {
          return hopc1 - hopc2;
        }
      } else if (p1 - p2 < 0) {
  ...
  /**
   * Message-Connection tuple comparator for the MaxProp routing module. Uses {@link
   * MaxPropComparator} on the messages of the tuples setting the "from" host for that message to be
   * the one in the connection tuple (i.e., path is calculated starting from the host on the other
  ...
  private class MaxPropTupleComparator implements Comparator<Tuple<Message, Connection>> {
    ...
    public int compare(Tuple<Message, Connection> tuple1, Tuple<Message, Connection> tuple2) {
      ...
      DTNHost from1 = tuple1.getValue().getOtherNode(MaxPropRouter.this.getHost());
      DTNHost from2 = tuple2.getValue().getOtherNode(MaxPropRouter.this.getHost());
