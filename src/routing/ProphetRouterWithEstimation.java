 ...
 *
 * <p>This version tries to estimate a good value of protocol parameters from a timescale parameter
 * given by the user, and from the encounters the node sees during simulation. Refer to Karvo and
 * Ott, <I>Time Scales and Delay-Tolerant Routing Protocols</I> Chants, 2008
 */
public class ProphetRouterWithEstimation extends ActiveRouter {
  ...
  public ProphetRouterWithEstimation(Settings s) {
    super(s);
    Settings prophetSettings = new Settings(ProphetRouterWithEstimation.PROPHET_NS);
    this.timescale = prophetSettings.getInt(ProphetRouterWithEstimation.TIME_SCALE_S);
    if (prophetSettings.contains(ProphetRouterWithEstimation.P_AVG_TARGET_S)) {
      this.ptavg = prophetSettings.getDouble(ProphetRouterWithEstimation.P_AVG_TARGET_S);
    } else {
      this.ptavg = ProphetRouterWithEstimation.DEFAULT_PTARGET;
    }
    if (prophetSettings.contains(ProphetRouterWithEstimation.BETA_S)) {
      this.beta = prophetSettings.getDouble(ProphetRouterWithEstimation.BETA_S);
    } else {
      this.beta = ProphetRouterWithEstimation.DEFAULT_BETA;
    }
    this.gamma = ProphetRouterWithEstimation.GAMMA;
    this.pinit = ProphetRouterWithEstimation.P_INIT;
  ...
  private boolean updateIET(DTNHost host) {
    ...
    if (this.meetings.containsKey(host)) {
      ...
      this.nrofSamples++;
      this.meanIET =
          (((double) this.nrofSamples - 1) / (double) this.nrofSamples) * this.meanIET
              + (1 / (double) this.nrofSamples) * timeDiff;
  ...
  private void updateParams() {
    ...
    ozeta = 0;
    cond = false;
    ntarg = (int) Math.ceil((double) this.timescale / this.meanIET);
    while (cond == false) {
      pstable = (1 - zeta) / (Math.exp(b * this.meanIET) - zeta);
      pavg = (1 / (b * this.meanIET)) * (1 - zeta * (1 - pstable)) * (1 - Math.exp(-b * this.meanIET));
      ...
      while (bcheck == false) {
        ...
        pstable = (1 - zeta) / (Math.exp(b * this.meanIET) - zeta);
        pnzero =
            Math.exp(-b * this.meanIET)
                * (1 - zeta)
                * ((1 - Math.pow(zeta * Math.exp(-b * this.meanIET), ntarg - 1))
                    / (1 - zeta * Math.exp(-b * this.meanIET)));
        pnone = Math.pow(zeta * Math.exp(-b * this.meanIET), ntarg) + pnzero;
  ...
  private void updateTransitivePreds(DTNHost host) {
    ...
    assert otherRouter instanceof ProphetRouterWithEstimation
  ...
  public RoutingInfo getRoutingInfo() {
    ...
    ri.addMoreInfo(
        new RoutingInfo(String.format("meanIET: %f\t from %d samples", this.meanIET,
            this.nrofSamples)));
    ri.addMoreInfo(new RoutingInfo(String.format("current gamma: %f", this.gamma)));
    ri.addMoreInfo(new RoutingInfo(String.format("current Pinit: %f", this.pinit)));
  ...
  private class TupleComparator implements Comparator<Tuple<Message, Connection>> {
    ...
    public int compare(Tuple<Message, Connection> tuple1, Tuple<Message, Connection> tuple2) {
      ...
      double p1 =
          ((ProphetRouterWithEstimation) tuple1.getValue().getOtherNode(
              ProphetRouterWithEstimation.this.getHost()).getRouter())
              .getPredFor(tuple1.getKey().getTo());
      // -"- tuple2...
      double p2 =
          ((ProphetRouterWithEstimation) tuple2.getValue().getOtherNode(
              ProphetRouterWithEstimation.this.getHost()).getRouter())
      ...
      if (p2 - p1 == 0) {
        ...
        return ProphetRouterWithEstimation.this.compareByQueueMode(tuple1.getKey(), tuple2.getKey());
      } else if (p2 - p1 < 0) {
        return -1;
      } else {
        return 1;
