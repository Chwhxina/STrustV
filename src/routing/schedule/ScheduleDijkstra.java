...
public class ScheduleDijkstra {
   ...
   * Constructor.
   *
   * @param oracle The schedule oracle all nodes are OK
   */
  ...
  private void initWith(Integer node, double time) {
    this.unvisited = new PriorityQueue<>(ScheduleDijkstra.PQ_INIT_SIZE, new DurationComparator());
    this.visited = new HashSet<>();
    this.prevHops = new HashMap<>();
   ...
   * @param time The time when the path starts
   * @return a shortest path between the source and destination nodes in a list of Integers or an
   *     empty list if such path is not available
   */
  public List<ScheduleEntry> getShortestPath(Integer from, Integer to, double time) {
    List<ScheduleEntry> path = new ArrayList<>();
    ...
    if (node != null) { // found a path
      ScheduleEntry prev = this.prevHops.get(to);
      while (prev.getFrom() != from) {
        path.add(0, prev);
        prev = this.prevHops.get(prev.getFrom());
      }
  ...
  private void relax(Integer node) {
    double timeNow = this.times.get(node);
    int to;
    double timeTo;

    for (ScheduleEntry se : this.oracle.getConnected(node, timeNow)) {
      to = se.getTo();
      ...
      if (timeTo < this.times.get(to)) {
        this.prevHops.put(to, se);
        this.setTime(to, timeTo);
      }
    }
  }
  ...
  private class DurationComparator implements Comparator<Integer> {
     ...
     *
     * @return -1, 0 or 1 if node1's time is smaller, equal to, or bigger than node2's
     */
    @Override
    public int compare(Integer node1, Integer node2) {
      double time1 = ScheduleDijkstra.this.times.get(node1);
      double time2 = ScheduleDijkstra.this.times.get(node2);
  ...
  private class TimeMap {
    ...
    /**
     * Returns the currently known smallest time one has a path for to the given node. If no time
     * value is found, returns {@link ScheduleDijkstra#INFINITY} as the value.
     *
    ...
    public double get(Integer node) {
      ...
      if (value != null) {
        ...
        return ScheduleDijkstra.INFINITY;
      }
    }
