...
public class ScheduleOracle implements Serializable {
   ...
   */
  public void addEntry(double start, int from, int via, int to, double duration) {
    List<ScheduleEntry> list = this.schedules.get(from);

    if (list == null) {
        /* first entry for the from */
      list = new ArrayList<>();
      this.schedules.put(from, list);
    }
  ...
  public void addEntry(double start, int from, int to, double duration) {
    this.addEntry(start, from, -1, to, duration);
  }
  ...
  public List<ScheduleEntry> getConnected(int from, double time) {
    List<ScheduleEntry> connected = new ArrayList<>();
    List<ScheduleEntry> all = this.schedules.get(from);
  ...
  public List<ScheduleEntry> getEntries() {
    ...
    for (List<ScheduleEntry> list : this.schedules.values()) {
