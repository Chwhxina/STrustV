...
public class MeetingProbabilitySet {
   ...
   *
   * @param maxSetSize Maximum size of the probability set; when the set is full, smallest values
   *     are dropped when new are added
   */
  public MeetingProbabilitySet(int maxSetSize, double alpha) {
    this.alpha = alpha;
    this.probs = new HashMap<>();
    if (maxSetSize == MeetingProbabilitySet.INFINITE_SET_SIZE || maxSetSize < 1) {
      this.probs = new HashMap<>();
      this.maxSetSize = MeetingProbabilitySet.INFINITE_SET_SIZE;
    } else {
  ...
  /**
   * Constructor. Creates a probability set with empty node-probability mapping and infinite set
   * size
   */
  public MeetingProbabilitySet() {
    this(MeetingProbabilitySet.INFINITE_SET_SIZE, 1);
  }

  /**
   * Constructor. Creates a probability set with equal probability for all the given node indexes.
   */
  public MeetingProbabilitySet(double alpha, List<Integer> initiallyKnownNodes) {
    this(MeetingProbabilitySet.INFINITE_SET_SIZE, alpha);
   ...
   * Normalize{P}</PRE>
   *
   * I.e., The probability of the given node index is increased by one and then all the
   * probabilities are normalized so that their sum equals to 1.
   *
  ...
  public void updateMeetingProbFor(Integer index) {
    ...
    if (this.probs.size() == 0) { // first entry
      this.probs.put(index, 1.0);
      return;
    }

    double newValue = this.getProbFor(index) + this.alpha;
    this.probs.put(index, newValue);

    /* now the sum of all entries is 1+alpha;
     * normalize to one by dividing all the entries by 1+alpha */
    for (Map.Entry<Integer, Double> entry : this.probs.entrySet()) {
      entry.setValue(entry.getValue() / (1 + this.alpha));
    ...
    if (this.probs.size() >= this.maxSetSize) {
      if (DEBUG) {
        core.Debug.p(
            "Probsize: " + this.probs.size() + " dropping " + this.probs.remove(smallestEntry.getKey()));
      }
    }
  }

  public void updateMeetingProbFor(Integer index, double iet) {
    this.probs.put(index, iet);
  }
  ...
  public double getProbFor(Integer index) {
    if (this.probs.containsKey(index)) {
      return this.probs.get(index);
    } else {
  ...
  public MeetingProbabilitySet replicate() {
    MeetingProbabilitySet replica = new MeetingProbabilitySet(this.maxSetSize, this.alpha);

    // do a deep copy
    for (Map.Entry<Integer, Double> e : this.probs.entrySet()) {
