 ...
 * Report file contains the count of connections that lasted for certain amount of time. Syntax:<br>
 * <code>time nrofContacts</code>
 */
public class ContactTimesReport extends Report implements ConnectionListener {
  /**
   * Granularity -setting id ({@value}). Defines how many simulated seconds are grouped in one
   * reported interval.
   */
  ...
  protected void addConnection(DTNHost host1, DTNHost host2) {
    ...
    assert !connections.containsKey(ci)
        : "Already contained " + " a connection of " + host1 + " and " + host2;
  ...
  protected class ConnectionInfo {
    ...
    /**
     * Should be called when the connection ended to record the time. Otherwise {@link
     * #getConnectionTime()} will use end time as the time of the request.
     */
    ...
    /**
     * Returns the time that passed between creation of this info and call to {@link
     * #connectionEnd()}. Unless connectionEnd() is called, the difference between start time and
     * current sim time is returned.
     *
     * @return The amount of simulated seconds passed between creation of this info and calling
     *     connectionEnd()
     */
