...
/**
 * Sampling report that counts the number of nodes in grid over the simulation area. Output format
 * is: G_x G_y average count_1, count_2, ... Where G_x and G_y are the coordinates of the grid
 * square [0, 1, ...] and count_n is the count during the nth sample.
 *
 * <p>The report can be configured to output a gnuplot script file that produces a heat map graph of
 * the node densities.
 *
 * @author teemuk
 */
public class NodeDensityReport extends SamplingReport {
  // ========================================================================//
  // Settings
  // ========================================================================//
  ...
  public static final String GNUPLOT_FILE_EXTENSION_SETTING = "gnuplotFileExtension";
  ...
  public static final String DEFAULT_GNUPLOT_TERMINAL = "png size 1024,768";
  ...
  public NodeDensityReport() {
    ...
    this.gnuplot = settings.getBoolean(GNUPLOT_SETTING, DEFAULT_GNUPLOT);
    this.onlyAverage = settings.getBoolean(ONLY_AVERAGE_SETTING, DEFAULT_ONLY_AVERAGE);
    this.gnuplotTerminal = settings.getSetting(GNUPLOT_TERMINAL_SETTING, DEFAULT_GNUPLOT_TERMINAL);
    this.gnuplotFileExtension =
        settings.getSetting(GNUPLOT_FILE_EXTENSION_SETTING, DEFAULT_GNUPLOT_FILE_EXTENSION);

    // Set up the sampling grid
    this.horizontalCount = settings.getInt(X_COUNT_SETTING, DEFAULT_X_COUNT);
    this.verticalCount = settings.getInt(Y_COUNT_SETTING, DEFAULT_Y_COUNT);
  ...
  private static double[][] getAverageDensities(
      final List<int[][]> samples, final int horizontalCount, final int verticalCount) {
  ...
  protected void sample(final List<DTNHost> hosts) {
    final int[][] sample = new int[this.horizontalCount][this.verticalCount];
  ...
  public void done() {
    ...
    final double[][] averages =
        getAverageDensities(this.samples, this.horizontalCount, this.verticalCount);

    for (int g_x = 0; g_x < this.horizontalCount; g_x++) {
      for (int g_y = 0; g_y < this.verticalCount; g_y++) {
        String line = "" + g_x + " " + g_y + " " + averages[g_x][g_y];
        if (!this.onlyAverage) {
  ...
  private void outputGnuplotSuffix(final int sampleCount) {
    super.write("EOD");
    super.write(
        "set output sprintf('" + this.runName + "-avrg." + this.gnuplotFileExtension + "')");
