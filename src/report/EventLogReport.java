...
/**
 * Report that creates same output as the GUI's event log panel but formatted like {@link
 * input.StandardEventsReader} input. Message relying event has extra one-letter identifier to tell
 * whether that message was delivered to final destination, delivered there again, or just normally
 * relayed (see the public constants).
 */
public class EventLogReport extends Report implements ConnectionListener, MessageListener {
  ...
  public void hostsConnected(DTNHost host1, DTNHost host2) {
    processEvent(
        StandardEventsReader.CONNECTION, host1, host2, null, StandardEventsReader.CONNECTION_UP);
  }

  public void hostsDisconnected(DTNHost host1, DTNHost host2) {
    processEvent(
        StandardEventsReader.CONNECTION, host1, host2, null, StandardEventsReader.CONNECTION_DOWN);
  }

  public void messageDeleted(Message m, DTNHost where, boolean dropped) {
    processEvent(
        (dropped ? StandardEventsReader.DROP : StandardEventsReader.REMOVE), where, null, m, null);
  }
