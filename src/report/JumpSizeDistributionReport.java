...
/**
 * Calculates the jump size probability density function. The measure is widely used for
 * characterising human mobility, e.g., in Marta C. Gonzaléz, César A. Hidalgo & Albert-László
 * Barabási, "Understanding individual human mobility patterns", Nature. 5. June 2008.
 *
 * <p>The simulation is sampled at fixed intervals (defined by the '{@link
 * SamplingReport#SAMPLE_INTERVAL_SETTING}' setting), and the distance from the previous sample
 * location is calculated for every active node. I.e., the result is the distribution of
 * displacements at the sampling interval
 *
 * <p>Note that picking too short sample interval will skew the distribution since it limits the
 * maximum observable displacement. For human mobility, the sample interval should be in the order
 * of a few hours to get prevent skewing.
 *
 * <p>Output line format is: "jump_length probability sample_count"
 *
 * @author teemuk
 */
public final class JumpSizeDistributionReport extends SamplingReport {

  // ========================================================================//
  // Constants
  // ========================================================================//
  /**
   * Setting to control the number of buckets (bins) used when calculating the probability density
   * function ({@value}). Default is the square root of the sample count.
   */
  public static final String BUCKET_COUNT_SETTING = "bucketCount";

  /** Set to {@code true} to output the raw jump sizes instead of the distributions ({@value}). */
  ...
  public JumpSizeDistributionReport() {
    ...
    final int defaultBucketCount = (int) Math.ceil(Math.sqrt(nodeCount * numSamples));
    this.bucketCount = settings.getInt(BUCKET_COUNT_SETTING, defaultBucketCount);
  ...
  // Private
  // ========================================================================//
  private static double maxJump(final double[][] samples, final int[] sampleCounts) {
    double max = 0;
  ...
  public final void done() {
    ...
    for (int i = 0; i < frequencies.length; i++) {
      ...
      final double density = 1.0 * frequencies[i] / (totalSamples * bucketWidth);
      final String densityLine = "" + jumpLength + " " + density + " " + frequencies[i];
    ...
    for (int i = 0; i < hosts.size(); i++) {
      ...
      if (host.isMovementActive()) {
        ...
        if (previousLocation != null) {
          final double jumpSize = currentLocation.distance(previousLocation);
