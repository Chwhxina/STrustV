...
/**
 * Message location report. Reports the location (coordinates) of messages. The messages that are
 * reported and the reporting interval can be configured.
 */
public class MessageLocationReport extends Report implements UpdateListener {
   ...
   * snapshot of message locations is created
   */
  public static final String GRANULARITY = "granularity";
  /**
   * Reported messages -setting id ({@value}). Defines the IDs of the messages that are reported
  ...
  /**
   * Creates a new snapshot of the message locations if "granularity" seconds have passed since the
   * last snapshot.
   *
  ...
  protected boolean isTracked(Message m) {
    return (this.reportedMessages == null || this.reportedMessages.contains(m.getId()));
  }
