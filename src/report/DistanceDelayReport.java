...
/**
 * Report for how far apart the nodes were when the message was sent and how long time & how many
 * hops it took to deliver it. Only messages created after the warm up period are counted. If
 * message is not delivered, its delivery time & hop count are reported as -1
 */
public class DistanceDelayReport extends Report implements MessageListener {
  /** Syntax of the report lines */
  public static final String SYNTAX = "distance at msg send, delivery time, hop count, MSG_ID";
  ...
  public void messageTransferred(Message m, DTNHost from, DTNHost to, boolean firstDelivery) {
    ...
    report(
        m.getId(),
        info.getLoc1().distance(info.getLoc2()),
        getSimTime() - info.getTime(),
        m.getHops().size() - 1);
  }
  ...
  public void newMessage(Message m) {
    ...
    this.creationInfos.put(
        m.getId(),
        new InfoTuple(
            getSimTime(), m.getFrom().getLocation().clone(), m.getTo().getLocation().clone()));
  }
