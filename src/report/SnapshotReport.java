...
/**
 * Node snapshot report superclass. Reports some characteristic of all (or only some, see {@link
 * #REPORTED_NODES}) nodes every configurable-amount-of seconds (see {@link #GRANULARITY}).
 */
public abstract class SnapshotReport extends Report implements UpdateListener {
  /**
   * Reporting granularity -setting id ({@value}). Defines the interval how often (seconds) a new
   * snapshot is created
   */
  public static final String GRANULARITY = "granularity";
  /**
   * Optional reported nodes (comma separated list of network addresses). By default all nodes are
   * reported.
   */
   ...
   * @param host
   */
  protected abstract void writeSnapshot(DTNHost host);

  /**
   * Creates a snapshot of all hosts by writing time stamp and calling {@link
   * #writeSnapshot(DTNHost)} for each host that is in the list of requested nodes
   *
