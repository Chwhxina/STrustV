...
/**
 * Link connectivity report connector that sends simulated connection events to the specified remote
 * server. The server needs to listen for TCP connections on the defined address and port. Report
 * connector sends a line based stream of time stamped ONE connectivity events, with connection
 * speed as last/extra value, formatted with {@link #format(double)}.
 */
public class ConnectivityReportConnector extends Report implements ConnectionListener {

  /**
   * Remote server address -setting id ({@value}). Defines the IP address or hostname of the server
   * where results are sent.
   */
  public static final String SERVER_ADDRESS_S = "address";
  /**
   * Server port -setting id ({@value}). Defines the TCP port number of the server where results are
   * sent.
   */
  public static final String SERVER_PORT_S = "port";
  /**
   * Write output also to file -setting id ({@value}). If set to false, results are not written to
   * local report file (default = true).
   */
  ...
  public ConnectivityReportConnector() {
    ...
    try {
      ...
      throw new SimError(
          "Can't connect to " + serverAddress + " port " + serverPort + ". Error: " + e);
    }
  }
  ...
  public void hostsConnected(DTNHost h1, DTNHost h2) {
    ...
    write(
        createTimeStamp() + " CONN " + connectionString(h1, h2) + " up " + format(con.getSpeed()));
  }
  ...
  /**
   * Creates and returns a String presentation of the connection where the node with the lower
   * network address is first
   *
